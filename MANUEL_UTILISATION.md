# 📘 Manuel d'Utilisation Personnel – GOAT Hedging & Recovery EA v4.1 (MT4 & MT5)
...
1. **Signal d'Entrée Initial (Breakout Pivot & Multi-Entrées) :**
   - Le robot surveille les plus hauts et plus bas récents sur la période choisie (`Swing_Length`).
   - Lorsqu'une bougie M5 clôture au-dessus du sommet pivot $\rightarrow$ Ouverture automatique de **`InitialTradeCount`** position(s) **BUY** en même temps.
   - Lorsqu'une bougie M5 clôture en-dessous du creux pivot $\rightarrow$ Ouverture automatique de **`InitialTradeCount`** position(s) **SELL** en même temps.

2. **Mécanisme de Protection (Hedging Recovery Zone Multi-Positions) :**
   - Si le marché prend la bonne direction, vos positions profitent directement.
   - Si le marché se retourne contre votre position initiale de `HedgeDistancePts` (ex: 300 points = $3.00 sur l'or) :
     - Le robot **garde la/les position(s) initiale(s) ouverte(s)**.
     - Il déclenche immédiatement **`HedgeTradeCount`** position(s) inverse(s) (**Hedge Counter Trade**) avec un volume multiplié (`HedgeLotMultiplier`, ex: x1.5).
...
- **InitialTradeCount (défaut : 1) :** Nombre de positions à ouvrir simultanément lors du signal de départ (ex: réglez à 2 ou 3 pour ouvrir 2 ou 3 BUY/SELL simultanés).
...
- **HedgeTradeCount (défaut : 1) :** Nombre de positions de couverture à ouvrir simultanément en cas de déclenchement (ex: réglez à 2 ou 3 pour ouvrir 2 ou 3 couvertures simultanées).
...
---
*GOAT Hedging & Recovery EA v4.1 – Disponible sur MT4 & MT5.*
